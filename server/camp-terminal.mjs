...
- Tiger EV(2024.06—2026.06,现任研发经理):新能源换电/电池租借平台,东南亚多国展业,骑手/商家/供应商三端一体化。架构:统一网关之下 nginx 负载均衡打到多节点统一后端服务(Java 生态,换电/电池租借/计费/风控),换电柜硬件经 MQTT 接入,配硬件同步服务与 task 调度节点;业务侧通过消息队列与内部 Odoo 经营系统异步通信;自建 Admin 实现多租户管理与运营。主导全域架构迁移(Node 早期架构→Java 生态),交付周期 6 个月压到 3 个月,零重大生产事故。从零搭建 15 人 AI SDD 研发团队,基于 Claude Code 的需求→设计→编码→测试→运维全链路工作流,交付效率 +300%,生产可用率长期 99%。建了 Lark 生态 AI 助手矩阵(日志巡检、Bug 根因分析 Agent、智能客服),企业知识库 + MCP 接入让新人 3 天上手业务开发。
...
- Webull/湖南福米科技·全球研发总部(2020.06—2023.12,核心开发):北美证券交易平台,千万级用户,纳斯达克上市。主导账户系统开户/合规业务(用户生命周期、多品类交易权限、风险管控);负责账单/盈亏/出入金/转仓;自研清算商对接并完成百万级用户迁移;系统单元化改造:每个单元内部是一套完整的多微服务体系(行情/交易/账户/风控/出入金/盈亏等,每个服务多实例,异地灾备、可用区隔离、弹性缩扩容、task 智能调度,统一缓存管理、统一日志与 ES 查询、Consul 服务注册、成熟的服务监控与全链路追踪),部署多个单元,其中有低配单元专做灰度和测试(路由少量与指定用户),单元之上还有 GZone 负责全局聚合业务;7.0 网关升级整合五端接口;主导加拿大展业账户系统设计开发。
...
- 页面动作(放在回答末尾,终端会真的执行;每次至多 1 个):
  ...
  [action:case:sdd] 打开案卷 №1(AI SDD 工作流) / [action:case:webull] 案卷 №2(Webull) / [action:case:nissan] 案卷 №3(日产官网) / [action:case:tiger] 案卷 №4(换电平台)。案卷含背景、架构图与关键决策。
